...
LPCOLLISIONEVENT CCollision::SweptAABB(LPGAMEOBJECT objSrc, DWORD dt, LPGAMEOBJECT objDest)
{
	...
	Vector2 mv = objSrc->GetVelocity();
	float mdx = mv.x * dt;
	float mdy = mv.y * dt;

	Vector2 sv = objDest->GetVelocity();
	float sdx = sv.x * dt;
	float sdy = sv.y * dt;
...
void CCollision::Process(LPGAMEOBJECT objSrc, DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	else
	{
		...
		Vector2 velocity = objSrc->GetVelocity();
		dx = velocity.x * dt;
		dy = velocity.y * dt;
		...
		if (colX != NULL && colY != NULL)
		{
			if (colY->t < colX->t)	// was collision on Y first ?
			{
				...
				//
				colX->isDestroyed = true;		// remove current collision event on X
			...
			else // collision on X first
			{
				...
				//
				colY->isDestroyed = true;		// remove current collision event on Y
