...
void CCamera::Update(float dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	if (followObject == NULL || followObject->IsDeleted() || followObject->GetState() == 0) {
	...
	{
		followObject->GetBoundingBox(playerAABB.left, playerAABB.top, playerAABB.right, playerAABB.bottom);
	...
	if (followObject->GetState() != MARIO_STATE_DEAD)
	{
		// Update camera to follow mario
		Vector2 playerPos = followObject->GetPosition();
